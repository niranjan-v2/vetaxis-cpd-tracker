...
export const generateQuiz = async (req, res, next) => {
  ...
  if (!user.isPro) {
    ...
    if (todayCount >= 3) {
      return next(
        errorHandler(403, "You've reached your daily quiz limit. Upgrade to AxisVet™ Pro for unlimited quizzes.")
      );
    }
  }
...
[
  ...
  try {
    ...
    res.status(200).json({ category, topic, difficulty, profession, questions });
  } catch (error) {
...
export const submitQuiz = async (req, res, next) => {
  ...
  try {
    ...
    const { passwordHash, ...userRest } = (updatedUser ?? await User.findById(req.user.id))._doc;
